		...
		else
		{
			...
			if($userdetail)
			{
				...
				if($sessiontrue==TRUE)
				{
					...
					//*************End Site Setting*************************************	
					//echo "ys";exit;	
					$condi = array('parent_id' => 0 );
					$data['totalcate']=$this->adminlogin->totalcate("products",$condi);
					$data['totalprod']=$this->adminlogin->totalprod("item");
					$data['services']=$this->adminlogin->totalprod("services");
					//echo "<pre>";print_r($data['totalprod'])	;exit;		
	...
	public function mydashboar()
	{
		...
		$sessiontrue=$this->session->userdata("user_name");
			...
			else
			{
				...
				/*$data['enquery']=$this->adminlogin->allinquery("inquety");*/
				$data['enquery']=$this->adminlogin->allinquery("order_table");
					$condi = array('parent_id' => 0 );
					$data['totalcate']=$this->adminlogin->totalcate("products",$condi);
					$data['totalprod']=$this->adminlogin->totalprod("item");
					$data['services']=$this->adminlogin->totalprod("services");
    ...
    public function setting()
    {
    	 $sessiontrue=$this->session->userdata("user_name");
			...
			{   $condi=array("section_id"=>$_GET['sectionid']);
				...
				$data['all']=$this->adminlogin->allsetting("setting_table");
				//echo "<pre>"; print_r($data['form_setting']);exit;
