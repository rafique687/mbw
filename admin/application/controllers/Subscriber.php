...
class Subscriber extends CI_Controller {
	...
	public function __construct()
	{
        ...
        $this->load->model('Subscrib_model');
	...
	{  
		 ...
		 $sessiontrue=$this->session->userdata("user_name");
			...
			{  
				...
				$data['customer'] = $this->Subscrib_model->allcustomer("newsletter");
				//$data['customer'] = $this->customer_modal->allcustomer("registeration");
				//echo "<pre>"; print_r($data['customer']);exit();
				$data['containt'] = "admin/view_subscriber.php";
	...
	{   
		...
		{
			$data['containt']="admin/composemailsubscriner.php";
		}
                ...
                else
                {
				       ...
				       // print_r($mail);exit;
				        ...
				        }else
				        {
				        	...
				        	if($fire=$this->Subscrib_model->stormaildetails("send_mailsubscriber",$emaildata))
				        	{
				        		redirect("Subscriber/maillist?success=Mail Send Successfully");
				        	}
...
{
	$data['all']=$this->adminlogin->allsetting("setting_table");	
	$data['mailsent']=$this->Subscrib_model->maillist("send_mailsubscriber");
	$data['containt']="admin/subscribsemaillist.php";
	$this->load->view("admin",$data);
...
public function deletecustomer()
{
	...
	$condi=array("sucribid "=>$_GET['id']);
	$fire=$this->customer_modal->delcustomer("newsletter",$condi);
	if($fire)
	{
		redirect("Subscriber");
	}
...
public function editcutomer()
{
	...
	$condi=array("sucribid"=>$_GET['id']);
	$data['editid']=$_GET['id'];
	$data['customer'] = $this->customer_modal->customerdetails("newsletter",$condi);
	//echo "<pre>"; print_r($data['customer']);exit;	
	$data['containt']="admin/editsubscriber.php";
	$this->load->view("admin",$data);					
}
				...
				if ($this->form_validation->run() == FALSE)
					{
					 	...
					 	$data['email'] = $_POST['email'];
					 	$data['mobile'] = $_POST['mobile'];
					 	$data['editid']=  $_POST['regi_id'];
						$data['containt'] = "admin/editsubscriber.php";
						$condi=array("sucribid"=>$_POST['regi_id']);
						$data['customer'] = $this->Subscrib_model->customerdetails("newsletter",$condi);
					$this->load->view('admin.php',$data);
					}
				else
					{
						...
						$fire= $this->Subscrib_model->updatecustomer("newsletter",$feeddata,$condi);
						if($fire)
						{
							redirect("Subscriber");
						}	
