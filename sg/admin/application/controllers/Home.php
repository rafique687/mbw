...
public function update_useaddress()
{
	...
	$data['country']=$this->home_model->country("contry");
	$data['state']=$this->home_model->country("state");
	$data['city']=$this->home_model->country("city");
	print_r(json_encode($data));exit;
...
if($fire){
	$mess= "Address Successfully Inserted";
...
{
$condi=array("adr_id"=>$_POST['adr_id']);
$data=array("user_id"=>$_POST['user_id'],
			"address"=>$_POST['address'],
			"country"=>$_POST['country'],
			"state"	=>$_POST['statid'],
...
if($fire){
	$mess= "Address Successfully Updated";
	print_r(json_encode($mess));exit;
...
if($fire)
{   
	$mess= "User Name Successfully Updated";
	print_r(json_encode($mess));exit;
}
else 
{
	$mess= "Problem to update Use Name";
	print_r(json_encode($mess));exit;
}
...
public function userorder()
	{
               ...
               //	$data['all']=$this->adminlogin->allsetting("setting_table");	
				$data['order']=$this->home_model->viewitem("order_table",$orderid);
