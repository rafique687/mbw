...
<form  action="<?php echo site_url()?>/product/inser_edititem" method="post" method="post" enctype="multipart/form-data"> 
	...
	<div class="form-group"> 
		<label>Select Sub Category</label>
    		<select class="form-control" name="subcate" id="subcate">
    			<?php foreach ($subcate as $scate) 
    			{?>
    			 	<option <?php if($scate['prod_id']==$product['sub_cate']){ echo 'selected';} ?> value="<?php echo $scate['prod_id'];?>"><?php echo $scate['product_name'];?></option>  
    	<?php   } ?>
			</select>
	</div>
	<div class="form-group"> 
		<label for="exampleInputEmail1">MRP.</label> 
		<input type="text" class="form-control" id="" name="mrp" placeholder="Product MRP" required="" value="<?php echo $product['mrp'];?>" > 
	...
	<div class="form-group"> 
		...
		<input type="text" class="form-control" id="" name="price" placeholder="Product Price" required="" value="<?php echo $product['price'];?>" > 
	 ...
	 <label>Stock Status</label>
			<select class="form-control" name="stockstatus" required="">
                <option 
                 <?php if($product['root_cate']==1){ echo 'selected';}?> value="1">In stock
                </option>
					<option <?php if($product['root_cate']==2){ echo 'selected';}?> value="2">Out of Stock
					</option>
			</select>
			<?php if(isset($error))
			{?>
	...
	<select class="form-control" name="offer" required="">
        <option <?php if($product['offer']=='yes'){ echo "selected";}?> value="yes">
        Yes
    	</option>
		<option <?php if($product['offer']=='no'){ echo "selected";}?>  value="no">
		No
		</option>
	</select>
	<?php if(isset($error))
